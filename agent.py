...
youtube_shorts_agent = LlmAgent(
    ...
    sub_agents=[scriptwriter_agent, visualizer_agent, formatter_agent, Agent_Code, Agent_Search],
)

# ----------------------- Top-level agent for ADK web -----------------------
root_agent = youtube_shorts_agent
agent = root_agent  # ✅ Must exist at top-level for ADK web discovery
...
if __name__ == "__main__":
    ...
    async def _ensure_session_created() -> None:
        await session_service.create_session(app_name=APP_NAME, user_id=USER_ID, session_id=SESSION_ID)

    asyncio.run(_ensure_session_created())
